"""
Structured targeted PLS exploration for the requested ROI atlas.

This fork re-runs the staged PLS screen on graph features derived from the revised
hemisphere-specific atlas saved under results/connectivity_new. The search stays
small enough for N = 14 while still testing the main modeling choices that matter:

1. ROI scope:
   - all requested ROIs
   - all requested ROIs with broad fallback cortex removed
   - motor-core ROIs
   - limbic-memory ROIs
   - posterior perceptual ROIs
   - frontal-association ROIs
...
All outputs are written under results/connectivity_new/PLS.
"""
...
from common_io import TASK_BEHAVIOR_COLUMN_SPECS, load_task_behavior_deltas, safe_slug


REPO_ROOT = Path(__file__).resolve().parents[2]
TMP = REPO_ROOT / "results" / "connectivity_new" / "GPT" / "tmp"
OUT_DIR = REPO_ROOT / "results" / "connectivity_new" / "PLS"

NODE_DELTA_PATH = TMP / "roi_graph_analysis_requested_atlas" / "node_metric_deltas_on_minus_off.csv"
MODULE_SUMM_PATH = TMP / "roi_graph_analysis_requested_atlas" / "module_integration_summary.csv"
...
LIMBIC_MEMORY_BASE_ROIS = [
    "Hippocampus",
    "Parahippocampal gyrus",
    "Amygdala",
]
POSTERIOR_PERCEPTUAL_BASE_ROIS = [
    "Precuneus",
    "Temporal cortex",
    "Fusiform cortex",
    "Lateral occipital cortex",
]
FRONTAL_ASSOCIATION_BASE_ROIS = [
    "Frontal Pole",
    "Frontal medial cortex (SMA/pre-SMA)",
    "Insular cortex",
]
NONSPECIFIC_BASE_ROIS = ["Cerebral Cortex"]
...
def _build_roi_schemes(graph_df: pd.DataFrame) -> dict[str, set[str]]:
    ...
    return {
        ...
        "broad_cortex_removed": set(sorted(all_rois))
        - {safe_slug(name) for name in NONSPECIFIC_BASE_ROIS},
        "motor_core": {safe_slug(name) for name in MOTOR_CORE_BASE_ROIS},
        "limbic_memory": {safe_slug(name) for name in LIMBIC_MEMORY_BASE_ROIS},
        "posterior_perceptual": {safe_slug(name) for name in POSTERIOR_PERCEPTUAL_BASE_ROIS},
        "frontal_association": {safe_slug(name) for name in FRONTAL_ASSOCIATION_BASE_ROIS},
    }
...
def build_model_configs(feature_tables: dict[str, pd.DataFrame]) -> list[ModelConfig]:
    ...
    baseline_specs = [
        ...
        (
            ...
            "graph_within_module",
            "full3",
            "Baseline all-ROI model emphasizing within-module integration.",
        ),
    ]
    ...
    for roi_scheme in broad_roi_schemes:
        roi_slugs = roi_schemes[roi_scheme]
        feature_cols = _select_feature_cols(feature_tables, "graph_participation", roi_slugs)
        for behavior_set in refined_behaviors:
            configs.append(
                _make_config(
                    stage="refined",
                    roi_scheme=roi_scheme,
                    neural_block="graph_participation",
                    behavior_set=behavior_set,
                    x_reduction="raw",
                    feature_cols=feature_cols,
                    description=(
                        f"Refinement: compact {roi_scheme} participation model without PCA "
                        "to test whether behavior choice or PCA was diluting the signal."
                    ),
                )
            )

    localized_base_rois = sorted(
        set(MOTOR_CORE_BASE_ROIS)
        | set(LIMBIC_MEMORY_BASE_ROIS)
        | set(POSTERIOR_PERCEPTUAL_BASE_ROIS)
        | set(FRONTAL_ASSOCIATION_BASE_ROIS)
        | set(NONSPECIFIC_BASE_ROIS)
    )
    for roi_name in localized_base_rois:
        ...
        for behavior_set in ["vmax_pmax", "full3"]:
            configs.append(
                _make_config(
                    ...
                    feature_cols=feature_cols,
                    description=(
                        "Localization within the requested atlas using single-ROI "
...
def _diagnose_model(row: pd.Series) -> tuple[str, str]:
    ...
    if (
        ...
        row["roi_scheme"] in {"all", "broad_cortex_removed", "motor_core", "limbic_memory", "posterior_perceptual", "frontal_association"}
        and roi_delta >= 0.12
    ):
        return (
        ...
        [
            {
                ...
                "choices_considered": "all, broad_cortex_removed, motor_core, limbic_memory, posterior_perceptual, frontal_association, single ROI",
                "explored_in_grid": True,
                "note": "Tests whether the requested atlas is better explained by broad distributed families or compact anatomical subsystems.",
            },
            {
                "parameter": "neural_block",
                "choices_considered": "graph_strength, graph_participation, graph_within_module, module_summary",
                "explored_in_grid": True,
                "note": "Separates graph topology and coarse module summaries. DCM is omitted here because the revised atlas no longer matches the old DCM node definition.",
            },
            {
...
- Best refined compact family model:
...
- Removing the broad fallback `Cerebral Cortex` compartment typically improved generalization relative to the all-ROI baselines.
- Restricting behavior from `full3` to `Vmax + Pmax` often improved both permutation strength and LOSO, consistent with timing and peak-vigor measures not always loading onto the same latent axis here.
- For compact refined models, removing PCA tended to help once the neural block was already small and anatomically interpretable.
- The strongest localized effect should be interpreted relative to this revised atlas, not as a direct continuation of the earlier control-family result.
...
- The most credible models are the compact graph-participation families that improve both significance and LOSO while staying anatomically interpretable.
- The best single-ROI model is best viewed as localization within the revised atlas search, not as an independent confirmatory result.
- No result here should be treated as fully definitive without an external replication or a preregistered follow-up on the winning compact family.
"""
...
def main() -> None:
    ...
    empty_dcm_df = pd.DataFrame({"subject": graph_df["subject"]})
    feature_tables = {"dcm": empty_dcm_df, "graph": graph_df, "module": module_df}

    behavior_df = load_task_behavior_deltas().copy()
    merged = (
        graph_df.merge(module_df, on="subject", how="outer")

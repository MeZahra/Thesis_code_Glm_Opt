#!/usr/bin/env python3
"""Create sham-comparison summary tables for Method 3 GVS outputs."""
...
DEFAULT_METHOD3_DIR = (
    ...
    / "GVS_effects"
    / "four_methods_v2"
    / "method3_off_to_on_normalization"
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--method3-dir",
        type=Path,
        default=DEFAULT_METHOD3_DIR,
        help="Path to the method3_off_to_on_normalization result directory.",
    )
...
def load_method3_tables(method3_dir: Path) -> tuple[pd.DataFrame, pd.DataFrame, Path, Path]:
...
def build_summary_table(stats_df: pd.DataFrame, observed_df: pd.DataFrame) -> pd.DataFrame:
    ...
    summary_df = summary_df.sort_values(
        ["distance_metric", "condition_factor", "condition_code"]
...
def write_csv_tables(summary_df: pd.DataFrame, tables_dir: Path) -> None:
    ...
    summary_df.loc[:, combined_columns].to_csv(
        tables_dir / "method3_statistics_vs_sham_summary.csv",
        index=False,
    )
    ...
    pd.DataFrame(significance_rows).to_csv(
        tables_dir / "method3_significant_gvs_vs_sham_fdr.csv",
        index=False,
    )
    ...
    for metric_name, metric_df in summary_df.groupby("distance_metric", sort=False):
        ...
        metric_df.loc[:, metric_columns].to_csv(
            tables_dir / f"method3_{_slugify(metric_name)}_statistics_vs_sham.csv",
            index=False,
        )
...
def render_metric_table(metric_df: pd.DataFrame, metric_name: str, out_path: Path) -> None:
    ...
    subtitle = (
        ...
        if sig_conditions
        else "FDR-significant vs sham: None"
    )
...
def write_plot_tables(summary_df: pd.DataFrame, plots_dir: Path) -> None:
    for metric_name, metric_df in summary_df.groupby("distance_metric", sort=False):
        render_metric_table(
            ...
            out_path=plots_dir / f"method3_{_slugify(metric_name)}_statistics_vs_sham_table.png",
        )


def main() -> None:
    args = parse_args()
    method3_dir = args.method3_dir.expanduser().resolve()
    stats_df, observed_df, tables_dir, plots_dir = load_method3_tables(method3_dir)

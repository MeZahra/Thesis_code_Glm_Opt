#!/usr/bin/env python3
"""Create sham-comparison summary tables for Method 2 GVS outputs."""
...
DEFAULT_METHOD2_DIR = (
    ...
    / "GVS_effects"
    / "four_methods_v2"
    / "method2_excess_distance_baseline_adjusted"
    / "baseline_full"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Read Method 2 excess-distance outputs and create compact summary "
    ...
    parser.add_argument(
        "--method2-dir",
        type=Path,
        default=DEFAULT_METHOD2_DIR,
        help="Path to the method2_excess_distance_baseline_adjusted baseline directory.",
    )
...
def load_method2_tables(method2_dir: Path) -> tuple[pd.DataFrame, pd.DataFrame, Path, Path]:
    tables_dir = method2_dir / "tables"
    plots_dir = method2_dir / "plots"
    stats_path = tables_dir / "method2_group_statistics.csv"
    observed_path = tables_dir / "method2_subject_observed_excess_distances.csv"

    if not stats_path.exists():
        raise FileNotFoundError(f"Missing statistics table: {stats_path}")
    if not observed_path.exists():
        raise FileNotFoundError(f"Missing observed excess-distance table: {observed_path}")
...
def build_summary_table(stats_df: pd.DataFrame, observed_df: pd.DataFrame) -> pd.DataFrame:
    observed_summary = (
        ...
        .agg(
            observed_mean_excess_distance="mean",
    ...
    summary_df["delta_vs_null_mean"] = summary_df["t_obs_mean_excess_distance"] - summary_df["null_mean"]
    ...
    summary_df = summary_df.sort_values(
        ["baseline_mode", "distance_metric", "medication", "condition_factor", "condition_code"]
...
def write_csv_tables(summary_df: pd.DataFrame, tables_dir: Path) -> None:
    ...
    summary_df.loc[:, combined_columns].to_csv(
        tables_dir / "method2_statistics_vs_sham_summary.csv",
        index=False,
    )

    significance_rows: list[dict[str, str]] = []
    for (baseline_mode, metric_name, medication), group_df in summary_df.groupby(
        ["baseline_mode", "distance_metric", "medication"],
        sort=False,
    ):
        ...
        significance_rows.append(
            {
                ...
                else "No significant GVS",
            }
        )
    pd.DataFrame(significance_rows).to_csv(
        tables_dir / "method2_significant_gvs_vs_sham_fdr.csv",
        index=False,
    )
    ...
    for metric_name, metric_df in summary_df.groupby("distance_metric", sort=False):
        ...
        metric_df.loc[:, metric_columns].to_csv(
            tables_dir / f"method2_{_slugify(metric_name)}_statistics_vs_sham.csv",
            index=False,
        )
...
def render_metric_table(metric_df: pd.DataFrame, metric_name: str, out_path: Path) -> None:
    ...
    display_df["MeanExcess"] = display_df["MeanExcess"].map(lambda value: f"{value:.3f}" if np.isfinite(value) else "NA")
    ...
    baseline_mode = str(metric_df["baseline_mode"].iloc[0]) if not metric_df.empty else "unknown"
    ...
    ax.set_title(
        f"Method 2 excess-distance statistics\n"
        f"{METRIC_TITLES.get(metric_name, metric_name)} | baseline={baseline_mode}\n"
        f"{subtitle}",
        fontsize=12,
        pad=18,
    )
...
def write_plot_tables(summary_df: pd.DataFrame, plots_dir: Path) -> None:
    for metric_name, metric_df in summary_df.groupby("distance_metric", sort=False):
        render_metric_table(
            ...
            out_path=plots_dir / f"method2_{_slugify(metric_name)}_statistics_vs_sham_table.png",
        )


def main() -> None:
    args = parse_args()
    method2_dir = args.method2_dir.expanduser().resolve()
    stats_df, observed_df, tables_dir, plots_dir = load_method2_tables(method2_dir)

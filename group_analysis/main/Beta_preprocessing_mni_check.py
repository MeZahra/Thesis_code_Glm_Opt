...
GLM_ROOT = os.path.join(REPO_ROOT, "results_glm")
OUT_ROOT = os.path.join(REPO_ROOT, "results_beta_preprocessed")
OUTPUT_ROOTS = [OUT_ROOT]
FORCE = os.getenv("BETA_PREPROCESS_FORCE", "0").lower() in {"1", "true", "yes"}
SUBJECT_FILTER = set(os.getenv("BETA_SUBJECTS", "").replace(",", " ").split())
SESSION_FILTER = set(os.getenv("BETA_SESSIONS", "").replace(",", " ").split())
RUN_FILTER = set(os.getenv("BETA_RUNS", "").replace(",", " ").split())
...
print(f"GLMsingle input root: {GLM_ROOT}", flush=True)
print(f"Beta output root: {OUT_ROOT}", flush=True)
...
def _load_run_onsets(sub, ses, run, num_trials):
    ...
    if not os.path.exists(path):
        raise FileNotFoundError(f"Missing go-times file: {path}")
    go_times = np.loadtxt(path, dtype=int)
    if go_times.ndim == 1:
        go_times = go_times[None, :]
    if run < 1 or run > go_times.shape[0]:
        raise ValueError(f"Run {run} requested but {path} has {go_times.shape[0]} row(s)")
    if go_times.shape[1] < num_trials:
        raise ValueError(f"{path} has {go_times.shape[1]} trial onsets, expected at least {num_trials}")
    return go_times[run - 1, :num_trials]

def _selected_subject(sub):
    if not SUBJECT_FILTER:
        return True
    return bool({sub, sub.removeprefix("pd"), f"sub-{sub}"} & SUBJECT_FILTER)

def _selected_number(value, selected):
...
for bold_path in bold_files:
    ...
    if not (
        _selected_subject(sub)
        and _selected_number(ses, SESSION_FILTER)
        and _selected_number(run, RUN_FILTER)
    ):
        continue
    selected_count += 1
    ...
    run_onsets = _load_run_onsets(sub, ses, run, num_trials)
    ...
    truncated_trials = 0
    for i, onset in enumerate(run_onsets):
        start = int(onset) - 1
        end = start + TRIAL_LEN
        if start < 0 or start >= num_timepoints:
            raise ValueError(f"Invalid onset {onset} for {tag} with {num_timepoints} timepoints")
        available_end = min(end, num_timepoints)
        if available_end < end:
            truncated_trials += 1
        n_available = available_end - start
        bold_data_reshape[:, i, :n_available] = masked_bold[:, start:available_end]
    if truncated_trials:
        print(f"{tag}: truncated {truncated_trials} trial window(s) at the end of the run", flush=True)
...
if selected_count == 0:
    raise RuntimeError("No BOLD files matched the requested beta-preprocessing filters.")
if processed_count == 0 and missing_glm_count:
    raise RuntimeError(
        f"No beta outputs were written because {missing_glm_count} selected GLMsingle "
        f"output file(s) were missing under {GLM_ROOT}."
    )
if processed_count == 0 and skipped_existing_count == selected_count:
    print("All selected beta-preprocessing outputs already exist.", flush=True)

...
# Zahra dataset paths
BOLD_ROOT = Path("/Data/zahra/bold_data")
RESULTS_ROOT = Path("/Data/zahra/data/results")
GLM_ROOT = Path("/Data/zahra/results")

ANAT_TEMPLATE = Path("/Data/zahra/anatomy_masks/MNI152_T1_2mm_brain.nii.gz")
BRAIN_MASK_TEMPLATE = Path("/Data/zahra/anatomy_masks/MNI152_T1_2mm_brain_mask.nii.gz")
CSF_MASK_TEMPLATE = Path("/Data/zahra/anatomy_masks/MNI152_T1_2mm_brain_seg_csf.nii.gz")
GRAY_MASK_TEMPLATE = Path("/Data/zahra/anatomy_masks/MNI152_T1_2mm_brain_seg_gm.nii.gz")
GO_TIMES_DIR = Path("/Data/zahra/go_times")  # not used by this script

# Subject/session (SUB/SES env vars; used for single-subject mode)
...
def _rank_rois_by_beta(summary, anat_img, anat_path, ref_img, out_path, atlas_threshold, label_patterns, assume_mni, summary_stat='percentile_95'):
    ...
    atlas_in_ref, registration_method = _align_atlas_to_reference(atlas_img, anat_img, anat_path, ref_img,
                                                                  out_path.parent, assume_mni=assume_mni, return_method=True)

    # Generate atlas QC diagnostics (optional).
    brain_mask_path = None
    anat_str = str(anat_path)
    if "T1w_brain.nii.gz" in anat_str:
        candidate = anat_str.replace('T1w_brain.nii.gz', 'T1w_brain_mask.nii.gz')
        if Path(candidate).exists():
            brain_mask_path = candidate
    elif BRAIN_MASK_TEMPLATE.exists():
        brain_mask_path = str(BRAIN_MASK_TEMPLATE)
    if brain_mask_path:
        _save_atlas_qc_plots(atlas_in_ref, labels, ref_img, brain_mask_path, out_path.parent, f'thr{atlas_threshold}', registration_method)
...
def _parse_args():
    ...
    parser.add_argument('--process-all', action='store_true', help='Process all subjects/sessions/runs found under /Data/zahra/bold_data.')
...
def main():
    ...
    args = _parse_args()
    if args.process_all:
        # Discover all available (sub, ses, run) from filenames (specific naming assumed).
        bold_files = sorted(BOLD_ROOT.rglob("*_task-mv_bold_corrected_smoothed_reg.nii.gz"))
        if not bold_files:
            raise FileNotFoundError(f"No BOLD files found under {BOLD_ROOT}")
        combos = set()
        for path in bold_files:
            parts = path.name.split("_")
            if len(parts) < 3:
                continue
            subj = parts[0]  # e.g., sub-pd004
            ses_part = parts[1]  # e.g., ses-1
            run_part = parts[2]  # e.g., run-1
            if not (subj.startswith("sub-") and ses_part.startswith("ses-") and run_part.startswith("run-")):
                continue
            try:
                sub_num = int(subj.split("sub-pd")[-1])
                ses_num = int(ses_part.split("ses-")[-1])
                run_num = int(run_part.split("run-")[-1])
            except ValueError:
                continue
            combos.add((sub_num, ses_num, run_num))

        script_path = Path(__file__).resolve()
        for sub_num, ses_num, run_num in sorted(combos):
            env = os.environ.copy()
            env["SUB"] = str(sub_num)
            env["SES"] = str(ses_num)
            cmd = [str(Path(os.sys.executable)), str(script_path), "--runs", str(run_num)]
    ...
    sub_label = f"sub-pd{sub:03d}"
    ses_dir = f"ses-{ses}"
    output_dir = Path(args.output_dir) if args.output_dir else (RESULTS_ROOT / sub_label / ses_dir)
    ...
    for r in selected_runs:
        fname = f"{sub_label}_ses-{ses}_run-{r}_task-mv_bold_corrected_smoothed_reg.nii.gz"
        matches = list(BOLD_ROOT.rglob(fname))
        if not matches:
            raise FileNotFoundError(f"Missing BOLD file: {fname} (searched under {BOLD_ROOT})")
        if len(matches) > 1:
            raise FileExistsError(f"Found multiple matches for {fname}: {matches}")
        bold_paths.append(matches[0])

    data_paths = {'bold': bold_paths[0], 'anat': ANAT_TEMPLATE, 'brain': BRAIN_MASK_TEMPLATE,
                  'csf': CSF_MASK_TEMPLATE, 'gray': GRAY_MASK_TEMPLATE}
    ...
    if missing:
        ...
        msg.append(f"BOLD_ROOT: {BOLD_ROOT.resolve()}")
    ...
    glm_dir = GLM_ROOT / sub_label / ses_dir / "GLMOutputs-mni-std"
    glmsingle_file = Path(args.glmsingle_file) if args.glmsingle_file else (glm_dir / f"run-{selected_runs[0]}" / "TYPED_FITHRF_GLMDENOISE_RR.npy")
    if not glmsingle_file.is_absolute():
        glmsingle_file = (Path.cwd() / glmsingle_file).resolve()
    if not glmsingle_file.exists():
        # Some runs may be stored without a run subfolder.
        fallback = (glm_dir / "TYPED_FITHRF_GLMDENOISE_RR.npy").resolve()
        if fallback.exists():
            glmsingle_file = fallback
    if not glmsingle_file.exists():
        tried = [str(Path(args.glmsingle_file).expanduser())] if args.glmsingle_file else [
            str((glm_dir / f"run-{selected_runs[0]}" / "TYPED_FITHRF_GLMDENOISE_RR.npy").resolve()),
            str((glm_dir / "TYPED_FITHRF_GLMDENOISE_RR.npy").resolve()),
        ]
        raise FileNotFoundError("GLMsingle output not found. Tried:\n  - " + "\n  - ".join(tried) +
            f"\nExpected base directory: {glm_dir}")
    ...
    if cached_beta_path.exists():
        ...
        if not skip_roi_ranking:
            ...
            _rank_rois_by_beta(mean_clean_active, anat_img=anat_img, anat_path=data_paths['anat'], ref_img=anat_img, out_path=roi_rank_path,
                                atlas_threshold=roi_atlas_threshold, label_patterns=roi_label_patterns, assume_mni=True, summary_stat=args.roi_stat)
        return
    ...
    del beta_glm
    gc.collect()
    if args.runs and len(selected_runs) > 1:
    ...
    if args.mask_indices:
        ...
        if beta.shape[0] == mask_indices_count:
            ...
            raise ValueError(f"Beta voxels ({beta.shape[0]}) do not match mask_indices ({mask_indices_count})"
    ...
    for run, trial_slice in run_slices:
        ...
        print(f"After filtering (run={run}): Beta Shape: {beta_volume_clean_2d.shape}, Bold shape: {clean_active_bold_keep.shape}")
        ...
        overlay_html_path_run = _with_tag(output_dir / f'clean_active_beta_overlay_sub{sub}_ses{ses}_run{run}.html', output_tag)
        overlay_snapshot_path_run = overlay_html_path_run.with_suffix(".png")
        _save_beta_overlay(mean_clean_active, anat_img=anat_img, out_html=str(overlay_html_path_run),
                           threshold_pct=overlay_threshold_pct, vmax_pct=overlay_vmax_pct, cut_coords=cut_coords,
        ...
        if not skip_roi_ranking:
            ...
            _rank_rois_by_beta(mean_clean_active, anat_img=anat_img, anat_path=data_paths['anat'], ref_img=anat_img,
                               out_path=roi_rank_path, atlas_threshold=roi_atlas_threshold, label_patterns=roi_label_patterns,
                               assume_mni=True, summary_stat=args.roi_stat)
